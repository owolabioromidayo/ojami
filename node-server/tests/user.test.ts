...
//   beforeAll(async () => {
//     app = await createApp(); // Create an instance of the app before running tests
//   });
...
//     const res = await request(app)
//       .get('/api/auth/users/1') // Replace with a valid user ID
//       .send();

//     // console.log(res.body);
//     expect(res.status).toBe(200);
//     expect(res.body.user).toHaveProperty('id');
//     expect(res.body.user).not.toHaveProperty('passwordHash'); // Ensure sensitive field is not returned
//   });

//   it('should return 404 for non-existent user', async () => {
//     const res = await request(app)
//       .get('/api/auth/users/9999') // Use an ID that does not exist
//       .send();
